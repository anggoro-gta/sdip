var com_github_culmat_jsTreeTable =  (function(){
	...
	function treeTable(table, is_expand){
		if(is_expand==null) is_expand = true;
		table.addClass('jsTT')
		...
		$("tr[data-tt-level]", table).each(function(index, tr) {
			...
			tr.trExpand = function(changeState){
				...
				$.each(this.trChildren, function(i, ctr) {
					if(doit) //$(ctr).slideDown('fast');
						$(ctr).css('display', 'table-row')
					ctr.trExpand()
				})
			}
			...
			$(tr).find('td:first-child').click(function() {
				$(this).parent()[0].trChildrenVisible ? $(this).parent()[0].trCollapse(true) : $(this).parent()[0].trExpand(true)
			})
		});
		if(!is_expand) $("tr[data-tt-level] td:first-child", table).click();
		return table
	}
