...
}  class master extends CI_Controller
{
    ...
    public function get_kecamatan_koordinat()
    {
        ...
        foreach ($kecamatan as $kec) {
            $data['features'][] = array( "type"=>"Feature", "geometry"=>array( "type"=>"Polygon", "coordinates"=>$kec->koordinat ? json_decode($kec->koordinat) : [] ), "properties"=>array( "id_unit"=>$kec->id_unit, "nama"=>$kec->nama, "kategori"=>$kec->kategori, "last_update"=>$kec->last_update, ) );
        }
    ...
    {
        $additional_options = isset($_GET['additional_options'])?
            $_GET['additional_options']:null;
    ...
    {
        $query = isset($_GET['q']) ? $_GET['q']:"";
        $result =  $this->m_master->get_select2_elemen(
            $query, 
            isset($_GET['kategori']) ? $_GET['kategori'] : null, 
            isset($_GET['id_parent']) ? $_GET['id_parent'] : null
        );
        $result = $result?$result:array();
